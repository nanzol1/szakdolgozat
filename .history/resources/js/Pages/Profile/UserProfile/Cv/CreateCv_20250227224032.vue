...
const countyNames = ref(props.postalDatas.county || []);
console.log(props.postalDatas.county);
...
const debounceSearch = _.debounce(async(element) => {
    ...
    if(elements[0] && elements[1].length > 3){
        try{
...
const computePostalDatas = computed(() => {
    if(props.postalDatas){
        Object.values(props.postalDatas?.map(data => {
            countyNames.value = data.county;
        }));
    }
});
</script>
