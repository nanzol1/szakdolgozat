...
const computeSkillLevel = computed(() => {
    return Object.values(props.formData.skills)?.map(elem => {
        getSkillLevel(elem.skill_level);
    });
});
        ...
        <div class="w-full" style="word-wrap: break-word;  max-width: 800px; width: 100%; padding: 20px; background-color: #fff;">
            ...
            <div style="margin-bottom: 15px;" v-if="formData.skills.length > 0 && formData.skills[0]?.skill_name !== ''">
                ...
                <ul style="list-style-type: none; padding: 0; margin: 0;">
                    <li style="font-size: 14px; margin: 5px 0;" v-for="(skll,index) in formData.skills" :key="index"><span v-if="skll.skill_name">{{skll.skill_name}}</span> - <span >{{ computeSkillLevel }}</span></li>
                </ul>
            </div>
