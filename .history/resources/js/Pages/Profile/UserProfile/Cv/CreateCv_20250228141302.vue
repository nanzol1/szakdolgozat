...
watch(() => [formData.zip, formData.city, formData.county], ([zip, city, county], [oldZip, oldCity, oldCounty]) => {
    if (zip !== oldZip) debounceSearch('zip', zip?.title || zip);
    if (city !== oldCity) debounceSearch('city', city?.title || city);
    if (county !== oldCounty) debounceSearch('county', county?.title || county);
});
