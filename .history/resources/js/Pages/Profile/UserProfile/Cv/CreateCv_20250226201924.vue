...
const generatePDF = async() => {
    const htmlContent = cvTemplate.value.outerHTML;
    ...
    a.href = url;
    a.download = `${formData.firstname}_CV.pdf`;
    a.click();
                ...
                <v-col>
                    <div class="flex">
                        <section class="w-1/2">
                            ...
                            <component v-else :is="steps[currentStepIndex].component" :key="currentStepIndex" :formData="formData" @updateForm="updateForm" @nextStep="nextStep" :prevStep="prevStep" @submitForm="submitForm" ref="cvTemplate" :generatePDF="generatePDF"></component>
                            </transition>

                            <div>
                                <button type="submit" class="dark:text-white" v-if="currentStepIndex === -1" @click="handleMainFormSubmit">Next</button>
                                <button type="submit" class="dark:text-white" v-if="currentStepIndex === -1" @click="generatePDF">Next</button>
                            </div>
                        </section>
                        <section class="w-1/2">
