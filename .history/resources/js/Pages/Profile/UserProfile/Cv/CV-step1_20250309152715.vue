    ...
    <v-form @submit.prevent="handleSubmit">
        <v-container>
          ...
          <template v-for="(exp, index) in exps" :key="index">
            ...
            <v-col cols="12" md="6">
              <n-date-picker v-model:value="exp.startdate" type="month" clearable size="large" placeholder="Kezdés ideje" :actions="[null]" :update-value-on-close="true"/>
            </v-col>
            <v-col cols="12" md="6">
              <n-date-picker v-model:value="exp.enddate" type="month" clearable size="large" placeholder="Végzés ideje"  :actions="[null]" :update-value-on-close="true"/>
            </v-col>
      ...
      <v-container>
        <v-row>
          <v-col cols="12">
              <v-btn type="button" @click="addExperience"  class="!bg-[#A67C52] hover:!bg-[#3A2618] !text-gray-50 hover:text-gray-50">Hozzáadás</v-btn>
            </v-col>
            <v-col cols="12" md="6" class="flex gap-4 flex-sm-wrap">
              <v-btn @click="prevStep" class="!bg-[#A67C52] hover:!bg-[#3A2618] !text-gray-50 hover:text-gray-50">Vissza</v-btn>
