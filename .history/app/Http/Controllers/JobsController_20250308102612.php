...
class JobsController extends Controller{
    public function index(Request $request){
        ...
        if(count($min_payment_q) <= 0 || (isset($min_payment_q[0]) && isset($max_payment_q[0]) && $min_payment_q[0] === $max_payment_q[0])){
    ...
    public function jobView($id){
        ...
        $jobs = JobVacancy::limit(8)->where('id','!=',$id)->get();
        if(auth()->guard('web')->check()){
            $applications = Application::where('user_id','=',request()->user()->id)->where('job_id','=',$id)->get();
        }
        $old_views = JobVacancy::select('views')->where('id','=',$id)->first()->toArray();
        $jobVacancy->update(['views'=> (int) implode($old_views) + 1]);
    ...
    public function applyForJob(Request $request){
        if(auth()->guard('web')->check()){
            if($request->job_id){
                ...
                $job = JobVacancy::where('id','=',$request->job_id)->with('companies','applicants')
                ->first();
                Log::logUserAction('jelentkezes','Jelentkezett erre a munkára -> ('.$validate['job_id'].')',auth('web')->user()->id);
                //event(new NewApplicant($job));
    ...
    public function saveListedJob(Request $request){
        ...
        if($datas){
            ...
            return redirect()->back()->with('success','Sikeresen el lett mentve!');
        }
    }

    public function showApplicants($jobId){
        $applications = Application::with(['user' => function($query){
            $query->select('id','name','email','phone_number','cv_file','introduction','birth_date','is_active');
        },
        'job_status','job_vacancy'])->where('job_id','=',$jobId)
        ->where('is_deleted','!=','1')
        ...
        ->get()
        ->toArray();
        $jobInfos = JobVacancy::where('id','=',$jobId)->first()->toArray();
        $statuses = JobStatus::all();
        return Inertia::render('Profile/CompanyProfile/Applicants/ViewApplicants',[
    ...
    public function showApplicant($jobId, $applicantId){
        ...
        $cleanedApps = array_map(function($apps) {
            return [
                ...
                'email' => $apps['user']['email'],
                'phone' => $apps['user']['phone_number'],
                'cv_file' => $apps['user']['cv_file'],
                'intro' => $apps['user']['introduction'],
                'birth_date' => $apps['user']['birth_date'],
            ];
        },$applications);
