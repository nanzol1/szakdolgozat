    ...
    >
        <nav v-if="canLogin" :class="['flex w-full justify-end bg-red-500 p-2 rounded-xl']">
            ...
            <template v-else>
                <Link
                    :href="route('/')"
                    class="rounded-md px-3 py-2 text-black ring-1 ring-transparent transition hover:text-black/40 focus:outline-none focus-visible:ring-[#FF2D20] dark:text-white dark:hover:text-white/80 dark:focus-visible:ring-white"
                >
                    Kezdőlap
                </Link>
                <Link
