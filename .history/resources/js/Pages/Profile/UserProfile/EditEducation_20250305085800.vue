...
const form = useForm({
    education_type: selectedEducation.education_type || null,
    institution_name: selectedEducation.institution_name || null,
    education_name: selectedEducation.education_name || null,
    education_level: selectedEducation.education_level || null,
    date_from: selectedEducation.date_from || null,
    date_to: selectedEducation.date_to || null,
    education_place: selectedEducation.education_place || null,
});
                    ...
                    <v-card class="education-card !border-2 !border-solid !border-stone-950 w-[90%] xl:w-[40%] lg:w-[90%] mx-auto" elevation="20">
                        ...
                        <div class="flex flex-col pa-5">
                            <div v-if="isEditing">
                                ...
                                <div>
                                    Időtartam: {{ new Date(selectedEducation.date_from).toLocaleDateString('HU') }} - {{ new Date(sedselectedEducationuc.date_to).toLocaleDateString('HU') }}
                                </div>
                                <div>
                            ...
                            <div v-else>
                                <v-form @submit.prevent="submit">
                                    <div>
                                        Végzettség típusa: <v-text-field label="Végzettség típusa" v-model="form.education_type"
                                        type="text" :error="!!form.errors.education_type" :error-messages="form.errors.education_type"
                                        ></v-text-field>
                                    </div>
                                    <div>
                                        Intézmény neve: <v-text-field label="Intézmény neve" v-model="form.institution_name"
                                        type="text" :error="!!form.errors.institution_name" :error-messages="form.errors.institution_name"
                                        ></v-text-field>
                                    </div>
                                    <div>
                                    ...
                                    <div>
                                        Időtartam: <v-text-field label="Végzettség megnevezése" v-model="form.education_name"
                                        type="text" :error="!!form.errors.education_name" :error-messages="form.errors.education_name"
                                        ></v-text-field>
                                        {{ new Date(seduc.date_from).toLocaleDateString('HU') }} - {{ new Date(seduc.date_to).toLocaleDateString('HU') }}
                                    </div>
                                    <div>
                            ...
                            <div class="flex justify-between flex-wrap">
                                ...
                                <div class="flex gap-2 md:gap-4 flex-wrap">
                                    <v-btn v-if="isEditing" type="submit" class="hover:bg-sky-500 hover:text-gray-50">Mentés</v-btn>
                                    <v-btn @click="isEditing = !isEditing" v-else class="hover:bg-sky-500 hover:text-gray-50">Szerkeszt</v-btn>
                                    <v-btn class="hover:bg-sky-500 hover:text-gray-50">Mégsem</v-btn>
                                </div>
                            </div>
                        </div>
                    </v-card>
