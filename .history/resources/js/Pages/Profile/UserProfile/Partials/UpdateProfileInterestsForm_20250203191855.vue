...
<template>
    <section class="w-full">
        ...
        <form @prevent.submit="form.post(route('profile.update.interests'))">
            ...
            <div v-for="categories in category" :key="category.name">
                {{ categories.name }}
                <Checkbox :value="categories.id" v-model:checked="checkboxes" :id="categories.id" @change="toggleSubcategories(category)">{{ categories.name }}</Checkbox>
