...
const generatePDF = async () => {
  ...
  html2canvas(element, {
    ...
    const imgHeight = 297; // A4 magasság

    // Számítsd ki a képarányt
    const canvasAspectRatio = canvas.width / canvas.height; // 1768 / 2246 ≈ 0.787
    const a4AspectRatio = imgWidth / imgHeight; // ≈ 0.707

    if (canvasAspectRatio > a4AspectRatio) {
      // Ha a canvas szélesebb, mint az A4
      const scaleFactor = imgWidth / canvas.width;
      pdf.addImage(imgData, "PNG", 0, (imgHeight - canvas.height * scaleFactor) / 2, imgWidth, canvas.height * scaleFactor);
    } else {
      // Ha a canvas magasabb, mint az A4
      const scaleFactor = imgHeight / canvas.height;
      pdf.addImage(imgData, "PNG", (imgWidth - canvas.width * scaleFactor) / 2, 0, canvas.width * scaleFactor, imgHeight);
    }
