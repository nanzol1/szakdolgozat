    ...
    <div class="bg-gray-50 text-black/100 dark:bg-black dark:text-white/50">
        ...
        >
            ...
            <div class="relative w-full max-w-2xl px-6 lg:max-w-7xl">
                <main class="mt-6">
                    <div class="lg:grid">
                        <div class="lg:text-xl mb-2 mt-4">
                            Ajánlott munkák
                        </div>
                        <div class="relative lg:max-w-7xl break-all md:mb-12" >
                            <v-sheet
                              elevation="8"
                              width="100%"
                            >
                              ...
                              >
                                ...
                                >
                                  <v-card
                                    :class="['ma-4 !bg-gray-300 lg:w-[350px] w-full']"
                                    height="200"
                                    @click="toggle(); selectedJob = featured;"
                                  >
                                    <Link :href="route('jobs.view',{id:featured.id})">
                                        <div class="flex flex-col h-full overflow-hidden">
                                            <div class="featured-card-title flex justify-between flex-wrap"><span>{{ featured.name }}</span><span>{{ featured.jobs_subcategory.name }}</span></div>
                                            <div class="mb-3 ml-2 h-20 overflow-hidden">{{ featured.description }}</div>
                                            <div class="absolute bottom-2 left-2"><div v-if="featured.companies.profile_pict ? hasPicutre.value = false : hasPicutre.value = true" class="w-12 h-12 rounded-xl overflow-hidden absolute left-0 bottom-0 max-sm:hidden"><img class="w-[100%] h-[100%] object-cover" :src="'storage/uploads/company_profile/'+featured.companies.id+'/'+featured.companies.profile_pict" alt="Profile picture"></div><div :class="['relative left-16 max-sm:left-0',{'left-0':featured.companies.profile_pict}]">{{ featured.companies.company_name }}</div></div>
                                        </div>{{ hasPicutre }}
                                    </Link>
                                  </v-card>
                                </v-slide-group-item>
