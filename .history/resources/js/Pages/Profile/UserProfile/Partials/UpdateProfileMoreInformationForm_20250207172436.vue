...
const debounceSearch = () => {
  if (searchQuery.value.length >= 3) {
    const response = axios.get('profile.institutions.search', {
      params: { institution_name: searchQuery.value },
    });
    institutions.value = response.data.institution_name;
  } else {
    institutions.value = [];
  }
};
    ...
    <section class="w-full">
        ...
        >
            ...
            <div>
                ...
                <div v-for="institution in institutions" :key="institution.id">{{ institution }}</div>
                <div v-if="searchQuery">Nincs találat</div>
