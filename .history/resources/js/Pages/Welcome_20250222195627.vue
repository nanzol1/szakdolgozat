...
const min_payment = ref(props.filters.min_payment || 0);
const max_payment = ref(props.filters.max_payment || 0);
...
watch([search, category,min_payment,max_payment], () => {
    currentPage.value = 1;
...
const fetchJobs = async () => {
    ...
    if(min_payment.value){
        params.min_payment = min_payment.value;
    }

    if(max_payment.value){
        params.max_payment = max_payment.value;
    }
                        ...
                        <div class="flex flex-col gap-6 sm:grid w-full lg:gap-2">
                            ...
                            <Link :href="route('jobs.view',{id:job.id})">
                                <v-card
                                    ...
                                    :title="job.name"
                                    :text="job.payment === null ? 'Nincs beállítva' : job.payment+' Ft'"
                                    link
                                >
                                </v-card>
                            </Link>
