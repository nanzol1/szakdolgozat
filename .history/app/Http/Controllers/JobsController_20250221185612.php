...
class JobsController extends Controller{
    ...
    public function saveListedJob(Request $request){
        ...
        if($datas){
            ...
            Log::logCompanyAction('update','Frissítette a munka adatait -> ('.$request->id.')',auth('web')->user()->id);
            if($job){
    ...
    public function updateStatus($jobId,$applicantId, $status){
        ...
        Log::logCompanyAction('update','Frissítette a jelentkező státuszát -> ('.$applicant['id'].'). Erre a státuszra -> ('.$applicant['status'].')',auth('web')->user()->id);
