...
class CVController extends Controller{
    ...
    public function generatePdf(Request $request){
        ...
        $options->set('isRemoteEnabled', true); // Engedélyezd a távoli erőforrások betöltését
        $options->set('defaultFont', 'Arial'); // Alapértelmezett betűtípus
    
        $dompdf = new Dompdf($options);
        $dompdf->loadHtml($html);
        $dompdf->setPaper('A4', 'portrait'); // Papírméret és orientáció
        $dompdf->render();
    
        // Hibakeresés: mentés fájlba
