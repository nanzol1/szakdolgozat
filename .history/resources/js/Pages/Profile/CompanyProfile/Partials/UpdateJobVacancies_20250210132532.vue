...
const computeEmployments = computed(() => {
    if(employments.value.length > 0){
        return console.log(Object.values(employments.value).filter(ep => ep && ep.id && ep.name).map(ep => ({
            id: ep.id,
            name:ep.name,
        })));
    }
    ...
    <section>
        ...
        >
            ...
            <div>
                ...
                <template v-for="types in employments" :key="types.id">
                    <label :for="types.id">{{ types.name }}</label>
                    <Checkbox v-model:checked="form.employment_type" :value="types.id"></Checkbox>
                </template>
                {{ computeEmployments }}
