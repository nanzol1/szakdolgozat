...
const checkMainCategory = (category) => {
    ...
    // Ellenőrizzük, hogy az alkategória már ki van-e jelölve
    const isSubCategoryChecked = checkboxes.value.includes(subCategoryId);

    // Ha az alkategória ki van jelölve, töröljük
    if (isSubCategoryChecked) {
        checkboxes.value = checkboxes.value.filter(id => id !== subCategoryId);

        // Ellenőrizzük, hogy maradt-e másik kijelölt alkategória
        const hasOtherSubcategories = props.categories[category.maincateg_id]?.subcategories.some(
            subcat => checkboxes.value.includes('s' + subcat.id)
        );

        // Ha nincs több kijelölt alkategória, töröljük a főkategóriát is
        if (!hasOtherSubcategories) {
            checkboxes.value = checkboxes.value.filter(id => id !== mainCategoryId);
        }
    } else {
        // Ha az alkategória nincs kijelölve, töröljük az összes többi alkategóriát ugyanabból a főkategóriából
        props.categories[category.maincateg_id]?.subcategories.forEach(subcat => {
            const otherSubCatId = 's' + subcat.id;
            checkboxes.value = checkboxes.value.filter(id => id !== otherSubCatId);
        });

        // Kijelöljük az új alkategóriát
        checkboxes.value.push(subCategoryId);

        // Ha a főkategória még nincs kijelölve, hozzáadjuk
