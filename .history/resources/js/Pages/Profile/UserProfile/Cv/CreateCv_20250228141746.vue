...
const userZipSelected = ref(false);
...
const debounceSearch = _.debounce(async (type, value) => {
    ...
    try {
        ...
        if (response.data.list && response.data.list.length > 0) {
            ...
            // Ha a felhasználó beírt egy ZIP-et, akkor NEM írjuk felül a ZIP-et
            if (!userZipSelected.value) {
...
// Figyeljük a ZIP-kód változását
watch(
    () => formData.zip,
    (zip, oldZip) => {
        if (zip !== oldZip) {
            userZipSelected.value = true; // A felhasználó beírt egy ZIP-et
            debounceSearch('zip', zip?.title || zip);
        }
    }
);

// Figyeljük a megye változását
watch(
    () => formData.county,
    async (county, oldCounty) => {
        if (county !== oldCounty) {
            userZipSelected.value = false; // Ha a megye változik, akkor engedhetjük a ZIP módosítását
            await debounceSearch('county', county);
        }
    }
);
