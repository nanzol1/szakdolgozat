...
class JobsController extends Controller{
    ...
    public function getMyJobs(){
        $applications = Application::with('user','job_vacancy','job_status')->where('user_id',auth()->user()->id)->get()->toArray();
        $jobs = array_filter($applications, function($app) {
            // Ellenőrizzük, hogy létezik-e 'job_vacancy' és hogy 'name' nem üres vagy null
            return isset($app['job_vacancy']) && !empty($app['job_vacancy']['name']);
        });
