...
const loadItems = async({page,itemsPerPage}) => {
    loading.value = true;
    await router.get(route("applied.jobs.load"), {page: page, perPage:itemsPerPage}, {
        preserveState: true,
        preserveScroll: true,
        replace:true,
        only: ["myjobs"],
...
<template>
    ...
    <div v-if="isShort">
        ...
        <template v-for="job in myjobs.data" :key="job.id">
            <v-row>
                <v-col cols="12">
                    <v-data-table-server :loading="loading" @update:options="loadItems" v-model:items-per-page="itemsPerPage" :items-length="itemsTotal" :headers="headers" :items="items"></v-data-table-server>
