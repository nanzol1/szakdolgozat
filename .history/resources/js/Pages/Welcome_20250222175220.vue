...
const filteredSubcategories = computed(() => {
    if(category.value.length === 0){
        return console.log(Object.values(props.categories).flatMap(cat => cat.subcategories || []));
    }else{
        return console.log(category.value.flatMap(id => props.categories[id]?.subcategories || []));
    }
});
                ...
                <main class="mt-6">
                    ...
                    <v-slide-x-reverse-transition>
                        <div v-if="filterShow" class="w-full lg:w-1/2 max-lg:my-3 lg:ml-10 bg-red-500">
                                <v-select v-for="(cats,index) in filteredSubcategories" :key="cats.id"
                                  v-model="category"
                                  :items="items"
                                  :label="cats.name"
                                  chips
