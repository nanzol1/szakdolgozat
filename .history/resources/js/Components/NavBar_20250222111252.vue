            ...
            <template v-else>
                    ...
                    <DisclosurePanel class="lg:hidden">
                      <div class="space-y-1 px-2 pt-2 pb-3">
                        <Link v-for="item in navigation" :key="item.name" as="a" :href="item.href" :class="[item.current ? 'text-gray-50dark:text-black dark:bg-gray-50' : 'text-gray-300 dark:hover:bg-gray-50 dark:hover:text-black', 'block rounded-md px-3 py-2 text-base font-medium']" :aria-current="item.current ? 'page' : undefined">{{ item.name }}</Link>
                      </div>
                    </DisclosurePanel>
            </template>
        </Disclosure>
