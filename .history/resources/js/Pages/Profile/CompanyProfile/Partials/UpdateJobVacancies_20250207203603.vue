...
const checkMainCategory = (category, subcategories = []) => {
    ...
    const subCategoryKey = 's' + category.subcateg_id;

    if (!checkboxes.value.includes(subCategoryKey)) {
        // Alkategória hozzáadása és a főkategória pipálása
        checkboxes.value.push(subCategoryKey);
        if (!checkboxes.value.includes(mainCategoryKey)) {
            checkboxes.value.push(mainCategoryKey);
        }
    } else {
        // Alkategória eltávolítása
        const index = checkboxes.value.indexOf(subCategoryKey);
        if (index !== -1) {
            checkboxes.value.splice(index, 1);
        }

        // Ellenőrizzük, hogy van-e még másik bejelölt alkategória a főkategóriában
        const hasOtherCheckedSubcategory = subcategories.some(sub => 
            checkboxes.value.includes('s' + sub.id)
        );

        // Ha nincs több bejelölt alkategória, akkor a főkategóriát is töröljük
            ...
            <div>
                ...
                <div v-for="cats in categories" :key="cats.id">
                    ...
                    <div v-for="subcats in cats.subcategories" :key="subcats.id">
                        ...
                        <Checkbox :value="'s'+subcats.id" v-model:checked="checkboxes" :id="subcats.name" @change="checkMainCategory(subcats,cats.subcategories)">{{ subcats.name }}</Checkbox>
                    </div>
                </div>
