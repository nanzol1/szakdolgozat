...
class BaseController extends Controller{
    public function index(Request $request){
        ...
        $min_payment = min(JobVacancy::select('payment')->where('payment','!=',null)->get()->toArray(),['payment'=>0]);
        $max_payment = max(JobVacancy::select('payment')->where('payment','!=',null)->get()->toArray(),['payment'=>1]);
        dd($min_payment);
