    ...
    public function updateEducation(Request $request){
        ...
        if($datas){
            $validated = Validator::make($datas, [
                ...
                'institution_name' => 'string|min:3|max:255',
                'education_name' => 'string|min:3|max:255',
                'education_level' => 'string',
                'date_from' => 'date',
                'date_to' => 'date',
                'education_place' => 'string',
            ],[
                'education_type.required' => 'Nem lehet üres a mező!',
                'institution_name.required' => 'Nem lehet üres a mező!',
                'education_name.required' => 'Nem lehet üres a mező!',
                'education_level.required' => 'Nem lehet üres a mező!',
                'date_from.required' => 'Nem lehet üres a mező!',
                'date_to.required' => 'Nem lehet üres a mező!',
                'education_place.required' => 'Nem lehet üres a mező!',
            ])->validate();

            if(array_key_exists('date_from',$validated)){
                $date_from = Carbon::createFromTimestampMsUTC($validated['date_from']);
                $date_from->setTimezone('Europe/Budapest');
                $validated['date_from'] = $date_from;
            }
            if(array_key_exists('date_to',$validated)){
                $date_to = Carbon::createFromTimestampMsUTC($validated['date_to']);
