        ...
        <Disclosure as="nav" v-if="canLogin" :class="['text-black flex flex-wrap w-full justify-center gap-5 bg-red-500 p-2 xl:text-lg lg:text-md md:text-xs sm:text-sm',{'rounded-xl': isFixed}]" v-slot="{ open }">
            ...
            <template v-else>
                    ...
                    <DisclosurePanel class="lg:hidden">
                      <div class="w-full space-y-1 px-2 pt-2 pb-3">
                        <Link v-for="item in navigation" :key="item.name" as="a" :href="item.href" :class="[item.current ? 'text-black bg-gray-50 dark:text-gray-50 dark:bg-gray-50' : 'text-gray-300 dark:hover:bg-gray-50 dark:hover:text-black', 'block rounded-md px-3 py-2 text-base font-medium']" :aria-current="item.current ? 'page' : undefined">{{ item.name }}</Link>
                      </div>
                    </DisclosurePanel>
            </template>
        </Disclosure>
