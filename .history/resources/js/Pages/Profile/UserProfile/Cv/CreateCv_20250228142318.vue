...
watch(() => formData.zip, async (data, oldData) => {
    if(data !== null && data.title && data.title !== 'null'){
        await debounceSearch('zip-'+data.title);
    }else{
        await debounceSearch('zip-'+data);
    }
});
/*watch(() => formData.city, async (data) => {
    if(data !== null && data.title && data.title !== 'null'){
        await debounceSearch('city-'+data.title);
    }else{
        await debounceSearch('city-'+data);
    }
});*/
/*watch(() => formData.county, async (data) => {
    if(data !== null && data.title && data.title !== 'null'){
        await debounceSearch('county-'+data.title);
    }else{
        await debounceSearch('county-'+data);
    }
});*/
...
const debounceSearch = _.debounce(async(element) => {
    ...
    if(!elements && elements[1].length < 3) return;

    if(elements[1].length > 3 && elements[1] !== 'null'){
        try{
            console.log('szűrés ment végbe');
