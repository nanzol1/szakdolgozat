...
const selectedInstitution = ref(false);Ł
...
watch(searchQuery, (newVal) => {
  if (!newVal && selectedInstitution.value) {
    // Ha a keresési mezőt manuálisan törölték, töröld a kiválasztott intézményt
...
const handlePick = (inst) => {
  if (inst) {
    ...
    searchQuery.value = inst.title; // Keresési mező frissítése a kiválasztott címkével
    isPicked.value = true;
  } else {
    // Ha törölték a kiválasztást
    form.institution_name = null;
    searchQuery.value = null;
    isPicked.value = false;
  }
                ...
                <v-col cols="12">
                    <v-form
                        @submit.prevent="form.post(route('profile.update.education'), {onSuccess: () => {
                            form.reset();
                            searchQuery.value = null; // Combobox tartalmának törlése
                            institutions.value = []; // Intézmény lista ürítése
                            isPicked.value = false; // Pick állapot visszaállítása
                        }})"
