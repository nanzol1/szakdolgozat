...
watch(() => formData.city, async (data, oldData) => {
    console.log('Régi - '+oldData+' Új - '+data);
    if(fromZip.value !== true){
        if(data && data.length > 3){
...
/*watch(() => formData.county, async (data) => {
    if(fromZip.value !== true){
        if(data && data.length > 3){
            ...
            console.log('County - Sima szűrés');
        }
        if(data !== null && data.title){
            await debounceSearch('county-'+data.title);
            console.log('County -  Title-es');
        }
    }
});*/
