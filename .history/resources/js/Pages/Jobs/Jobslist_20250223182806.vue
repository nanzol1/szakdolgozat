...
const asd = computed(() => {
    const asdd = Object.values(props.categories).map(a => a.subcategories);
    asdd.forEach(element => {
        return element.map(s => s.subcat_maincid);
    });
});
                    ...
                    <div class="w-1/5 mr-5 pr-5">
                        ...
                        <v-expand-transition>
                            <div v-if="isFilterShow">
                                ...
                                <div v-for="(cats,index) in categories" :key="cats.mid"  class="mb-5">
                                    <label :for="cats.mid">{{ cats.name }}</label>
                                    <Checkbox v-model:checked="category" :value="cats.mid" :id="cats.mid"></Checkbox>
                                    <Checkbox v-model:checked="subcategory" :value="cats.subcategories.subcat_id" :id="cats.subcategories.subcat_keyid"></Checkbox>
                                    {{ cats.subcategories[index][index].subcat_id }}
                                </div>
