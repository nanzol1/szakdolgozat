...
const debounceSearch = _.debounce(async (type, value) => {
    if (!value || value.length < 3 || value === 'null') return;

    try {
        const response = await axios.get(route('cv.getcodes', { datas: `${type}-${value}` }));
        if (response.data.list && response.data.list.length > 0) {
            const data = response.data.list[0]; // Csak az első találatot vesszük

            if (!userEditing.value.county) {
                formData.county = data.county || '';
            }
            if (!userEditing.value.city) {
                formData.city = data.place_name || '';
            }
            if (!userEditing.value.zip) {
...
// Figyeljük a felhasználói beavatkozást
const setUserEditing = (field) => {
    userEditing.value[field] = true;
    setTimeout(() => {
        userEditing.value[field] = false;
    }, 2000); // 2 másodperc múlva visszaállítjuk
};

// Egyetlen `watch`, ami figyeli az összes mezőt
watch(
    () => [formData.zip, formData.city, formData.county],
    ([zip, city, county], [oldZip, oldCity, oldCounty]) => {
        if (zip !== oldZip) {
            setUserEditing('zip');
            debounceSearch('zip', zip?.title || zip);
        }
        if (city !== oldCity) {
            setUserEditing('city');
            debounceSearch('city', city?.title || city);
        }
        if (county !== oldCounty) {
            setUserEditing('county');
            debounceSearch('county', county?.title || county);
        }
    }
);
