...
const debounceSearch = _.debounce(async () => {
    ...
    if(searchQuery.value){
        ...
        if (searchQuery.value.length >= 2 || searchQuery.value.title && searchQuery.value.title.length >= 2) {
            const response = await axios.get(route('profile.institutions.search'), {
                params: { institution_name: searchQuery.value?.title ?? searchQuery.value?.title},
            });
...
const handlePick = (inst) => {
    if (inst) {
        if (typeof inst === 'object') {
            ...
            // Ha szöveget ír, ne állítsuk be az intézmény nevét
            // Lehet, hogy itt inkáccább üríteni kell, vagy más logikát alkalmazni
            isPicked.value = false;
        }
    } else {
