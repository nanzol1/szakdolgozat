...
const handleCategoryBtn = (cats) => {
    if(isCategory.value === false){
        isCategory.value = true;
        return Object.values(cats).flatMap(a => [{id:a.subcat_id,name:a.subcat_name}]);
    }else{
                    ...
                    <div class="w-1/5 mr-5 pr-5">
                        ...
                        <v-expand-transition>
                            <div v-if="isFilterShow">
                                ...
                                <div v-for="cats in categories" :key="cats.mid"  class="mb-5">
                                    <label :for="cats.mid">{{ cats.name }}</label>
                                    <Checkbox v-model:checked="category" :value="cats.mid" :id="cats.mid"></Checkbox>
                                    <button @click="handleCategoryBtn">asd</button>
                                    <div v-for="s in handleCategoryBtn(cats.subcategories)" :key="s.subcat_id" v-if="isCategory">
                                        <label :for="s.subcat_id">{{ s.subcat_name }}</label>
                                        <Checkbox v-model:checked="subcategory" :value="s.subcat_id" :id="s.subcat_id"></Checkbox>
                                    </div>
                                </div>
                                <v-range-slider
                                  v-model="payment"
