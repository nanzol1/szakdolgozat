...
onMounted(() => {
    ...
    isAppliedArray.forEach(element => {
        if(element.job_id === props.job.id || element.user_id === user.id){
...
const revokeApplication = () => {
    router.delete(route('jobs.apply.revoke',{id:props.job.id}),{
        job_id: props.job.id,
    }/*,{
        onSuccess: () => {
            isApplied.value = false;
...
<template>
    ...
    <button @click="sendApplication" :disabled="isApplied">{{computeApplied}}</button>
