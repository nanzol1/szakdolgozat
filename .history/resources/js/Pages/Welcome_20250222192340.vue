...
const clearInput = (searchText) => {
    console.log(getCategoryItems.value.some(item => item.name === searchText));
  if (searchText && !getCategoryItems.value.some(item => item.name === searchText)) {
    // Ha a beírt szöveg nem egyezik egy elemmel sem, töröljük
    searchText = '';
  }
};
const filterItems = (item, queryText, itemText) => {
  ...
  // Csak azokat az elemeket jeleníti meg, amelyek egyeznek a keresési feltétellel
