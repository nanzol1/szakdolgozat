...
<template>
    <div class="elevation-4" style="width: 100%; height: 100%; background-color: #fff;">
        <div class="w-full relative" style="word-wrap: break-word;  max-width: 800px; width: 100%; background-color: #fff;">
            <div class="w-full" style="text-align: center; margin-bottom: 15px;">
                ...
                <p style="font-size: 14px; margin: 5px 0;" v-if="props.isAddition"><span v-if="formData.address">{{formData.address}},</span> <span v-if="formData.city">{{formData?.city.title || formData.city}},</span> <span v-if="formData.county">{{formData?.county.title || formData.county}}</span> <span v-if="formData.zip">{{ formData.zip }}</span></p>
            </div>
            ...
            <div style="margin-bottom: 15px;" v-if="formData.skills.length > 0 && formData.skills[0]?.skill_name !== ''">
                ...
                <ul style="list-style-type: none; padding: 0; margin: 0;">
                    <li style="font-size: 14px; margin: 5px 0;" v-for="(skll,index) in formData.skills" :key="index"><span v-if="skll.skill_name">{{skll.skill_name}}</span><span v-if="skll.skill_name && computeSkillLevel[index]"> -</span> <span>{{ computeSkillLevel[index] }}</span></li>
                </ul>
            </div>
