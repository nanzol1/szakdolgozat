...
const checkMainCategory = (category) => {
    ...
    // Ha az alkategória be van pipálva
    if (checkboxes.value.includes(subCategoryId)) {
        // Töröljük az összes más alkategóriát ugyanabból a főkategóriából
        checkboxes.value = checkboxes.value.filter(item => {
            return !item.startsWith('s') || item === subCategoryId;
        });

        // Hozzáadjuk a főkategóriát, ha még nincs benne
        if (!checkboxes.value.includes(mainCategoryId)) {
            checkboxes.value.push(mainCategoryId);
        }
    } else {
        // Ha az alkategóriát töröljük, akkor ellenőrizzük, hogy van-e még más alkategória bepipálva ugyanabban a főkategóriában
        ...
        // Ha nincs más alkategória bepipálva, töröljük a főkategóriát is
        if (!hasOtherSubcategories) {
            checkboxes.value = checkboxes.value.filter(item => item !== mainCategoryId);
        }
    }

    // Ellenőrizzük az összes főkategóriát, és töröljük azokat, amelyekhez nem tartozik alkategória
    const mainCategories = checkboxes.value.filter(item => item.startsWith('m'));
    mainCategories.forEach(mainCat => {
        const mainCatId = mainCat.replace('m', '');
        const hasSubcategories = checkboxes.value.some(item => {
            return item.startsWith('s') && item.split('s')[1] === mainCatId;
        });

        // Ha nincs alkategória a főkategóriához, töröljük a főkategóriát
        if (!hasSubcategories) {
            checkboxes.value = checkboxes.value.filter(item => item !== mainCat);
        }
    });
};
...
onMounted(() => {
    ...
    checkboxes.value.forEach(item => {
        if (item.startsWith('s')) {
            const category = Object.values(props.categories)?.(cat => cat.subcategories).find(sub => 's' + sub.id === item);
            if (category) {
                checkMainCategory(category);
            }
        }
    });
});
