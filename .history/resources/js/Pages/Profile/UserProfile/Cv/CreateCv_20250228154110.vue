...
watch(() => [formData.zip,formData.county,formData.city], async ([zip,county,city]) => {
    if(zip.length > 3){
        await debounceSearch(zip?.title || zip);
    }
    if(county.length > 3){
        await debounceSearch(county?.title || county);
    }
    if(city.length > 3){
        await debounceSearch(city?.title || city);
    }
},{deep:true});
