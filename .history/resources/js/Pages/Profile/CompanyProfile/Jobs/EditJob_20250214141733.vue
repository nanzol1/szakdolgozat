...
const checkMainCategory = (category) => {
    ...
    if (checkboxes.value.includes(subCategoryId)) {
        // Ha az alkategória már ki van pipálva, akkor töröljük
        checkboxes.value = checkboxes.value.filter(item => item !== subCategoryId);

        // Megnézzük, hogy maradt-e másik alkategória ugyanebben a főkategóriában
        const hasOtherSubcategories = checkboxes.value.some(item =>
            item.startsWith('s') &&
            props.categories[category.maincateg_id].subcategories.some(subcat => item === 's' + subcat.id)
        );

        // Ha nincs több alkategória bepipálva, töröljük a főkategóriát is
        if (!hasOtherSubcategories) {
            checkboxes.value = checkboxes.value.filter(item => item !== mainCategoryId);
        }
    } else {
        // Ha egy új alkategóriát jelölünk be, előbb töröljük az összes többi alkategóriát ugyanabból a főkategóriából
        checkboxes.value = checkboxes.value.filter(item =>
            !(item.startsWith('s') && props.categories[category.maincateg_id].subcategories.some(subcat => item === 's' + subcat.id))
        );

        // Bejelöljük az új alkategóriát
        checkboxes.value.push(subCategoryId);

        // Hozzáadjuk a főkategóriát is, ha még nincs benne
