...
const showPopUp = async (applicantId) => {
    try{
        ...
        console.log(response.data.applicant.status_id < 2);
        if(response.data.applicant.status_id <= 1){
            const asd = await router.patch(route('cprofile.job.updatestatus',{id:props.job.id,jelentkezoId:applicantId,statusId:2}));
        }
