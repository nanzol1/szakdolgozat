...
const registeredUsers = computed(() => {
    ...
    props.users?.users.forEach(element => {
        console.log(new Date(Date.now()).toLocaleDateString().split('/')[1]);
        console.log(new Date(element.created_at).toLocaleDateString().split('/')[1] >= 1 && new Date(element.created_at).toLocaleDateString().split('/')[1] <= 30);
    });
});
