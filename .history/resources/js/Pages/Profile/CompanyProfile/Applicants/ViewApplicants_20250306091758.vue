                        ...
                        <section>
                            ...
                            <v-overlay v-model="isPopup" class="align-center justify-center" width="100%">
                                <v-card class="education-card pa-4 !border-2 !border-solid !border-stone-950 w-[90%] xl:w-[50%] lg:w-[90%] mx-auto">
                                    ...
                                    {{ statuses }}
                                    <div class="flex gap-[10em] justify-evenly progress-container">
                                        <div v-for="(sts, index) in statuses" :key="index" class="step">
                                            <div v-if="sts.name !== 'Jelentkezett'" class="relative">
                                                <input type="radio" v-model="status" :name="sts.name" :value="sts.id" 
                                                :class="['!border-1 !border-solid !border-black']"
                                                :id="'radio'+index">
                                                <label :for="'radio'+index">{{ sts.name }}</label>
                                                <div v-if="index < statuses.length"
                                                class="custom-line"
                                                :class="{'filled':status >= index + 1}">
                                                {{ status }}
                                                </div>
                                            </div>
                                        </div>
