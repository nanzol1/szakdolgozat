    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if($request->logintype === 'munkavallalo'){
            ...
            if($user && Auth::guard('web')->attempt($credentials)) {
                ...
                return redirect()->intended(route('dashboard', absolute: false))->with('Sikeres bejelentkezés!');;

            }else{
                return redirect()->intended(route('login',absolute:false))->with('Sikertelen bejelentkezés!');
            }
            ...
            if($company && Auth::guard('company')->attempt($credentials)) {
                ...
                return redirect()->intended(route('cdashboard', absolute: false))->with('Sikeres bejelentkezés!');;
            }else{
                return redirect()->intended(route('login',absolute:false))->with('Sikertelen bejelentkezés!');
            }
            ...
            if($admin && Auth::guard('admin')->attempt($credentials)) {
                ...
                return redirect()->intended(route('admin.login', absolute: false))->with('Sikertelen bejelentkezés!');
            }
        }

        return redirect()->back()->with('error','Hibás email cím vagy jelszó!');
    }
