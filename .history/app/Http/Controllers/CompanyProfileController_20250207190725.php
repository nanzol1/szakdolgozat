    ...
    public function update(Request $request): RedirectResponse
    {
        ...
        if($request->hasFile('profile_pict')){
            if($user->profile_picture){
                Storage::disk('public')->delete('uploads/company_profile/'.$user->profile_pict);
            }

            $path = $request->file('profile_pict');
        ...
        if($request->user()->update($validated)){
            if($request->hasFile('profile_pict')){
                $path->store('uploads/company_profile/'.$request->user,'public');
