...
const navigation = [
  ...
  { name: 'Bejelentkezés', href: route('login'), current: page.url === '/login' ? true : false,isVisible: isLoggedIn === true ? 'hidden' : ''},
  { name: 'Regisztráció', href: route('register',{type:''}), current: page.url === '/register' ? true : false , isVisible: page.props.auth.user !== null ? 'hidden' : ''},
  { name: 'Profilom', href: is_company === 1 ? route('cprofile.edit') : route('profile.edit'), current: page.url === '/profile' ? true : false,isVisible: isLoggedIn === true ? '' : 'hidden'},
]
