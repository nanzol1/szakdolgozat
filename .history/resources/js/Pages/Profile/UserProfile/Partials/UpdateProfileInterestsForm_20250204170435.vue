...
// Számított property a kijelölt elemek neveinek lekéréséhez
const selectedNames = computed(() => {
    return checkboxes.value.map((id) => {
        // Keresd meg a kategóriát vagy alkategóriát az id alapján
    ...
    }).filter(Boolean); // Szűrd ki a null értékeket
});
...
<template>
    <section class="w-full">
        ...
        <form @submit.prevent="submitForm">
            <!-- Kijelölt elemek nevei -->
            <div>Érdekeltségeim: {{ selectedNames.join(', ') }}</div>

            <!-- Checkbox-ok -->
            <div v-for="categories in category" :key="categories.id">
                <label :for="categories.id">{{ categories.name }}</label>
                <Checkbox
                    :value="'m' + categories.id"
                    v-model:checked="checkboxes"
                    :id="categories.id"
                    @change="toggleSubcategories(categories)"
                >
                    {{ categories.name }}
                </Checkbox>
