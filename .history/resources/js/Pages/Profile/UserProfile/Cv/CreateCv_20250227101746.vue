...
const generatePDF = async () => {
  ...
  html2canvas(element, {
    ...
    useCORS: true,
    logging: true,
    scrollY: -window.scrollY, // Biztosítja, hogy az egész oldal rögzítésre kerüljön
    ...
    const pageHeight = 297; // A4 magasság

    // A pdfFixHeight kiszámítása
    const pdfRatio = pageHeight / canvas.height;
    const pdfFixHeight = canvas.height * pdfRatio;

    let position = 0;

    if (pdfFixHeight > pageHeight) {
      // Többoldalas PDF létrehozása
      while (position < pdfFixHeight) {
        pdf.addImage(imgData, "PNG", 0, -position, imgWidth, pdfFixHeight); // A kép megfelelő részének hozzáadása
        position += pageHeight;

        if (position < pdfFixHeight) {
          pdf.addPage(); // Új oldal hozzáadása, ha még van tartalom
        }
      }
    } else {
      // Egyoldalas PDF, ha a kép elfér egy oldalon
      pdf.addImage(imgData, "PNG", 0, 0, imgWidth, pdfFixHeight);
    }
            ...
            <v-row>
                <v-col>
                    <div class="flex">
                        ...
                        <section class="w-1/2 flex flex-col flex-1" style="box-sizing: border-box; min-height: 297mm;">
                            <!--<div v-html="templateContent"></div>-->
                            <component ref="cvTemplate" class="flex flex-col flex-1-1 fill-height" :is="tempaltes[0].component" :formData="formData" :isAddition="isAddition" style="width: 100%; height: 100%; min-height: 297mm;"></component>
                        </section>
                    </div>
                </v-col>
            </v-row>
