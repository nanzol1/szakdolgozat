...
class ProfileController extends Controller
{
    ...
    public function edit(Request $request): Response
    {
        ...
        $category = [];
        $user = User::select('jobs_interests')->where('id','=', $request->user()->id)->first();
    ...
    public function updateInterests(Request $request){
        ...
        $interests['jobs_interests'] = $request->interests;
        dd(in_array('m1',$interests));
        if($interests){
            $request->user()->update($interests);
        }
    }
