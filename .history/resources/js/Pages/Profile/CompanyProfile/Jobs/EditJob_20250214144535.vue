...
const checkMainCategory = (category) => {
    ...
    if (checkboxes.value.includes(subCategoryId)) {
        ...
        const hasOtherMaincategories = checkboxes.value.some(item => {
            return item.startsWith('m') && item !== mainCategoryId && item.split('m')[1] === category.subcateg_id.toString();
        });
        if(hasOtherMaincategories){
