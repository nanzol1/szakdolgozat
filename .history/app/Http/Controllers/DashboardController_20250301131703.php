...
class DashboardController extends Controller{
    ...
    public function getAppliedJobsTable(Request $request)
    {
        ...
        ->select([
            'applications.id as application_id', // Alias az applications.id-hez
            'applications.user_id',
            'applications.job_id',
            'applications.status',
            'job_vacancies.id as job_vacancy_id', // Alias a job_vacancies.id-hez
            'job_vacancies.name as job_name',
            'job_vacancies.active as job_active',
            'jobs_subcategories.id as subcategory_id', // Alias a jobs_subcategories.id-hez
            'jobs_subcategories.name as subcategory_name',
            'job_statuses.id as status_id', // Alias a job_statuses.id-hez
        ...
        if ($request->has('sortBy')) {
            ...
            switch ($request->sortBy) {
                case 'name':
                    $applications->orderBy('job_vacancies.name', $sortDirection);
                    break;
        
                case 'subcat':
                    $applications->orderBy('jobs_subcategories.name', $sortDirection);
                    break;
        
                case 'status':
                    $applications->orderBy('job_statuses.name', $sortDirection);
                    break;
        
                case 'active':
                    $applications->orderBy('job_vacancies.active', $sortDirection);
                    break;
