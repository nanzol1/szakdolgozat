...
class JobsController extends Controller{
    public function index(Request $request){
        ...
        $workSchedules = $request->input('work_schedules'); // [2, 3, 4]

        $results = YourModel::where(function ($query) use ($workSchedules) {
            foreach ($workSchedules as $schedule) {
                $query->orWhereJsonContains('work_schedules', $schedule);
            }
        })->get();
        if($request->filled('work_schedules')){
            $query->whereIn('work_schedules', $request->work_schedules);
        }
        ...
        foreach($jobs as $item){
            $cleanedJobs[$item->id] = [
                ...
                'name'=> $item->name,
                'active' => $item->active,
                'position_name' => $item->jobs_category->name,
                'sub_position_name' => $item->jobs_subcategory->name,
                'description' => $item->description,
                'created_by' => $item->companies->company_name,
                'maincateg_id' => $item->jobs_category->id,
                'sucateg_id' => $item->jobs_subcategory->id,
                'company_avatar' => $item->companies->profile_pict,
                'company_id' => $item->created_by,
            ];
        }
