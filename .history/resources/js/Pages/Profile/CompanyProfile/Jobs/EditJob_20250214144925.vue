...
const checkMainCategory = (category) => {
    ...
    // Ha egy alkategóriát választunk, akkor a főkategóriáját is kiválasztjuk
    if (checkboxes.value.includes(subCategoryId)) {
        // Távolítsuk el az összes többi főkategóriát
        checkboxes.value = checkboxes.value.filter(item => !item.startsWith('m'));
        // Adjuk hozzá a főkategóriát
        if (!checkboxes.value.includes(mainCategoryId)) {
            checkboxes.value.push(mainCategoryId);
        }
    } else {
        // Ha az alkategóriát eltávolítjuk, akkor ellenőrizzük, hogy van-e még más alkategória ugyanabból a főkategóriából
        ...
        // Ha nincs más alkategória, akkor távolítsuk el a főkategóriát is
        if (!hasOtherSubcategories) {
            checkboxes.value = checkboxes.value.filter(item => item !== mainCategoryId);
        }
    }

    // Ellenőrizzük, hogy csak egy főkategória legyen kiválasztva
    const mainCategories = checkboxes.value.filter(item => item.startsWith('m'));
    if (mainCategories.length > 1) {
        // Ha több főkategória van kiválasztva, akkor csak az utolsót hagyjuk meg
        checkboxes.value = checkboxes.value.filter(item => item === mainCategories[mainCategories.length - 1]);
    }
