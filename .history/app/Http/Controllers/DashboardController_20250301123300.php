    ...
    public function getAppliedJobsTable(Request $request)
    {
        ...
        if ($request->has('sortBy')) {
            ...
            switch ($request->sortBy) {
                case 'name':
                    $query->orderBy(JobVacancy::select('name')
                        ->whereColumn('job_vacancies.id', 'applications.job_vacancy_id'), 
                        $sortDirection);
                    break;
                    
                case 'subcat':
                    $query->orderBy(JobsSubcategory::select('name')
                        ->whereColumn('jobs_subcategories.id', 'job_vacancies.subcateg_id'), 
                        $sortDirection);
                    break;
