...
const navigation = [
  ...
  { name: 'Bejelentkezés', href: route('login'), current: page.url === '/login' ? true : false,isVisible: isLoggedIn ? false : true},
  { name: 'Regisztráció', href: route('register',{type:''}), current: page.url === '/register' ? true : false , isVisible: isLoggedIn ? false : true},
  { name: 'Profilom', href: is_company === 1 ? route('cprofile.edit') : route('profile.edit'), current: page.url === '/profile' ? true : false,isVisible: isLoggedIn ? true : false },
]
...
<template>
    ...
    {{ navigation.isVisible }}
        <Disclosure as="nav" v-if="canLogin" :class="['text-gray-950 flex flex-wrap w-full justify-center gap-5  bg-sky-500 p-2 xl:text-xl lg:text-md md:text-xs sm:text-sm',{'rounded-xl': isFixed}]" v-slot="{ open }">
                ...
                <div class="flex w-full items-center justify-center sm:items-stretch sm:justify-start">
                  <div class="hidden lg:ml-3 lg:block">
                    <div class="flex space-x-4">
                      <Link v-for="item in navigation" :key="item.name" :href="item.href" :class="[item.current ? 'dark:text-black text-gray-50' : 'hover:text-white dark:text-white dark:hover:text-black transition', item.isVisible ? 'hidden' : '' , 'xl:px-3 xl:py-2 text-sm font-medium']" :aria-current="item.current ? 'page' : undefined">{{ item.name }}</Link>
                    </div>
                  </div>
                </div>
