...
const checkMainCategory = (category) => {
    ...
    // Ha az alkategória már ki van jelölve, akkor töröljük azt
    if (checkboxes.value.includes(subCategoryId)) {
        checkboxes.value.splice(checkboxes.value.indexOf(subCategoryId), 1);

        // Ellenőrizzük, hogy maradt-e másik alkategória az adott fő kategóriában
        ...
        // Ha nincs több kijelölt alkategória, a fő kategória pipáját is eltávolítjuk
        if (!hasOtherSubcategories) {
            const mainIndex = checkboxes.value.indexOf(mainCategoryId);
            if (mainIndex !== -1) {
                checkboxes.value.splice(mainIndex, 1);
            }
        }
    } else {
        // Először távolítunk el minden más alkategóriát az adott fő kategóriából
        props.categories[category.maincateg_id]?.subcategories.forEach(subcat => {
            const subCatId = 's' + subcat.id;
            const index = checkboxes.value.indexOf(subCatId);
            if (index !== -1) {
                checkboxes.value.splice(index, 1);
            }
        });

        // Új alkategória bejelölése
        checkboxes.value.push(subCategoryId);

        // Fő kategória bejelölése, ha még nincs
