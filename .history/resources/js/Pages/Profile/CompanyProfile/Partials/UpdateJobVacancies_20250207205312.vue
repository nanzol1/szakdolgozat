...
const checkMainCategory = (category) => {
    console.log(props.categories);  // Nézd meg, mi a típus és az érték

    const mainCategoryId = 'm' + category.maincateg_id;
    const subCategoryId = 's' + category.id;

    if (checkboxes.value.includes(subCategoryId)) {
        // Ha hozzáadunk egy alkategóriát, és még nincs fent a fő kategória, hozzáadjuk
        if (!checkboxes.value.includes(mainCategoryId)) {
            checkboxes.value.push(mainCategoryId);
        }
    } else {
        // Ellenőrizzük, hogy valóban tömb a categories
        if (Array.isArray(props.categories)) {
            // Ha kivettük az alkategóriát, nézzük meg, van-e még másik ugyanebben a fő kategóriában
            const hasOtherSubcategories = props.categories.some(cat => 
                cat.maincateg_id === category.maincateg_id &&
                cat.subcategories.some(subcat => checkboxes.value.includes('s' + subcat.id))
            );

            // Ha nincs másik, akkor vegyük ki a fő kategóriát is
            ...
            console.error('props.categories nem tömb!');
        }
    }
};
