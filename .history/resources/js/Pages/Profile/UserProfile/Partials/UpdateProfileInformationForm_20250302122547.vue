...
// 1. Figyeld a dátumválasztó megnyitását
const handlePickerOpen = (isOpen) => {
  isPickerOpen.value = isOpen
}

// 2. DOM manipuláció a megnyitáskor
watch(isPickerOpen, (isOpen) => {
  if (!isOpen) return
  
  // Várakozás a DOM renderelésére (timeout szükséges)
  setTimeout(() => {
    const weekdays = document.querySelectorAll('.n-date-picker-weekdays__day') 
    
    // Angol -> Magyar rövidítések
    const huShortMap = {
      'Su': 'V',
      'Mo': 'H',
      'Tu': 'K',
  ...
  }, 100) // 100ms késleltetés a DOM stabilizálódásához
})
