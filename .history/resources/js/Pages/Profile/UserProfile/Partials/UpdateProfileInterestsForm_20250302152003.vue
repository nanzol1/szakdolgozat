        ...
        <v-form @submit.prevent="submitForm">
            <transition-group tag="div" name="categories" mode="out-in">
                <div v-for="cats in category" :key="cats.id"  class="mb-5">
                    ...
                    <Checkbox v-model:checked="checkboxes" :value="'m'+cats.id" :id="cats.id"></Checkbox>
                    <transition-group tag="div" name="checkboxes">
                        <div v-for="subcat in getSubCategories(cats.subcategories)" :key="subcat.id" class="mt-3" v-if="checkboxes.includes(cats.id)">
                            <label :for="'s'+subcat.id">{{ subcat.name }}</label>
                            <Checkbox v-model:checked="checkboxes" :value="'s'+subcat.id" :id="'s'+subcat.id"></Checkbox>
                        </div>
                    </transition-group>
                </div>
            </transition-group>
