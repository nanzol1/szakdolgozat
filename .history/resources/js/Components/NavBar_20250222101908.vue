...
<template>
    <div :class="['navbar w-full',{'fixed-nav':isFixed}]" :style="{transform: isFixed ? 'translateY(0)' : 'translateY(10px)'}">
        <header
        :class="['grid items-center gap-2 top-0']"
        
    >
        <nav v-if="canLogin" :class="['flex flex-wrap w-full justify-center gap-5 bg-red-500 p-2 xl:text-lg lg:text-md md:text-xs sm:text-sm']">
            <Link
