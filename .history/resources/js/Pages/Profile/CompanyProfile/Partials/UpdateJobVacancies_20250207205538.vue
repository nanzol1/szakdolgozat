...
const checkMainCategory = (category) => {
    ...
    // Ellenőrizzük, hogy a checkboxok között szerepel-e a választott subcategory
    if (checkboxes.value.includes(subCategoryId)) {
        // Ha már bent van, de nincs a fő kategória, akkor hozzáadjuk a fő kategóriát
        if (!checkboxes.value.includes(mainCategoryId)) {
            checkboxes.value.push(mainCategoryId);
        }
    } else {
        // Ha eltávolítjuk az alkategóriát, ellenőrizzük, hogy van-e másik alkategória a fő kategóriában
        const hasOtherSubcategories = Object.values(props.categories).some(cat => 
            cat.maincateg_id === category.maincateg_id &&
            cat.subcategories.some(subcat => checkboxes.value.includes('s' + subcat.id))
        );

        // Ha nincs másik alkategória, akkor vegyük ki a fő kategóriát is
    ...
    <section>
        ...
        >
            ...
            <div>
                ...
                <div v-for="(cats, catKey) in categories" :key="cats.id">
