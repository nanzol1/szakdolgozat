...
const imageUrl = `/storage/images/your-image.jpg`;

  var url = 'path_to_your_pdf.pdf';

  // PDF betöltése
  pdfjsLib.getDocument(url).promise.then(function(pdf) {
    // Az első oldal betöltése
    pdf.getPage(1).then(function(page) {
      var scale = 1.5;
      var viewport = page.getViewport({ scale: scale });

      // Canvas létrehozása a megjelenítéshez
      var canvas = document.createElement('canvas');
      var context = canvas.getContext('2d');
      canvas.height = viewport.height;
      canvas.width = viewport.width;

      // Oldal renderelése a canvas-ra
      page.render({
        canvasContext: context,
        viewport: viewport
      });

      // A canvas hozzáadása a DOM-hoz
...
<template>
    ...
    <section class="w-full">
        <div>
            <p class="text-white mb-2">Feltöltött CV</p>
            <iframe :src="'../storage/uploads/cv/'+user.cv_file+'#toolbar=0'" alt="Uploaded CV" class="w-full h-[300px]" loading="lazy" frameborder="0"></iframe>
        </div>
