...
const getSubcategoryItems = computed(() => {
  return Object.values(props.categories).map(cat => ({
    id: cat.id,    // Az érték (id)
    name: cat.name // A megjelenítendő szöveg
  }));
});
</script>
...
<template>
    ...
    <div class="bg-gray-50 text-black/100 dark:bg-black dark:text-white/50 pb-10">
        ...
        chips
        multiple
        item-title="name"  <!-- A megjelenítendő szöveg (kategória neve) -->
        item-value="id"    <!-- A visszaadott érték (kategória id-je) -->
        return-object="false" <!-- Csak az id-t adja vissza -->
      ></v-combobox>
                        </div>
                    </v-slide-x-reverse-transition>
                    </div>
