...
class JobsController extends Controller{
    ...
    public function getListedJobs(){
        $listedJobs = JobVacancy::with(['companies' => function($query){
            $query->select('id','active','name','address','description','created_by','maincateg_id','subcateg_id');
            },
            'employment_type' => function($query){
                $query->select('id','name','','address','description');
            },
            'work_schedules',
            'jobs_category',
            'jobs_subcategory',
