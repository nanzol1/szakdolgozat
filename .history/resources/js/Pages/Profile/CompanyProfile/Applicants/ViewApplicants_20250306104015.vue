...
const updateStatus = async (id,applicantId,statusId) => {
    ...
    selectedApplicant.value.status_id = status.value;
};
            ...
            <v-container>
                <v-row>
                    <v-col cols="12">
                        <section>
                            Jelentkező(k)
                            <div v-for="apps in applications" :key="apps.id">
                            {{ apps }}
                                <v-card :class="['mb-3 pa-3 listedjobs-card',{'hover-effect':clickedDiv === apps.id}]" @click="clickedDiv = apps.id">
                                    ...
                                    <div :class="['listedjobs-card-actions',{'hover-effect-actions':clickedDiv === apps.id}]">
                                        ...
                                        <v-btn :href="'/storage/uploads/cv/'+apps.user.cv_file" @click="isCVDownloaded" :download="apps.user.name+'.pdf'" class="hover:bg-sky-500 hover:text-gray-50" v-if="apps.user.cv_file">Önéletrajz letöltése</v-btn>
                                    </div>
                                </v-card>
                            </div>
                            <v-overlay v-model="isPopup" class="align-center justify-center" width="100%">
                                <v-card class="education-card pa-4 !border-2 !border-solid !border-stone-950 w-[90%] xl:w-[50%] lg:w-[90%] mx-auto">
                                    ...
                                    <div v-if="selectedApplicant.cv_file">
                                        <v-btn :href="'/storage/uploads/cv/'+selectedApplicant.cv_file" @click="isCVDownloaded" :download="selectedApplicant.name+'.pdf'" class="hover:bg-sky-500 hover:text-gray-50" v-if="selectedApplicant.cv_file">Önéletrajz letöltése</v-btn>
                                    </div>
                                    {{ selectedApplicant }}
                                    <div class="flex w-full mt-3 justify-evenly" v-if="selectedApplicant.status_id >= 4">
                                        <v-btn class="hover:bg-sky-500 hover:text-gray-50" @click="updateStatus(props.job_id,selectedApplicant.user_id,4)">Elfogad</v-btn>
                                        <v-btn class="hover:bg-sky-500 hover:text-gray-50" @click="updateStatus(props.job_id,selectedApplicant.user_id,5);">Elutasít</v-btn>
