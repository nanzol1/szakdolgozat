...
const page = usePage();
const hasCvFile = computed(() => !!user.cv_file);

const fetchProfile = async () => {
    await Inertia.reload({only:['user']});
};

let interval;
    onMounted(() => {
      interval = setInterval(fetchProfile, 5000); // 5 másodpercenként frissít
    });
...
<template>
    ...
    <section class="w-full">
        ...
        <PrimaryButton @click="removeCV">Törlés</PrimaryButton>
