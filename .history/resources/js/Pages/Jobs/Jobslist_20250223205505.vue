                    ...
                    <div class="w-full xl:w-1/2 xl:flex-1 max-xl:mt-5">
                            <div v-for="job in jobs" :key="job.id">
                                <Link :href="route('jobs.view',{id:job.id})">
                                    <v-card
                                      class="mb-3"
                                      :subtitle="job.position_name + ' / ' + job.sub_position_name"
                                      elevation="2"
                                    >
                                        <template v-slot:prepend>
                                            <v-icon icon="$vuetify" color="error"></v-icon>
                                        </template>
                                        <template v-slot:append>
                                            <v-icon icon="$vuetify" color="warning"></v-icon>
                                        </template>
                                      <template v-slot:title>
                                        <span class="font-weight-black">{{ job.created_by}}</span>
                                      </template>
                                      <v-card-subtitle>
                        ...
                        <v-pagination v-model="currentPage" active-color="blue" ellipsis="..." :length="totalPages" @update:model-value="fetchJobs"></v-pagination>
                        <v-btn variant="tonal" @click="startFilters" :class="['mr-3 !text-stone-50 !bg-gray-500 hover:!bg-sky-500 hover:scale-105 mb-5',{'!bg-sky-500':isFilterShow}]">
                            Szűrés
                        </v-btn>
                        <v-btn variant="tonal" @click="resetFilters" :class="['mr-3 !text-stone-50 !bg-gray-500 hover:!bg-sky-500 hover:scale-105 mb-5',{'!bg-sky-500':isFilterShow}]">
                            Alaphelyzetbe
                        </v-btn>
                    </div>
                </div>
