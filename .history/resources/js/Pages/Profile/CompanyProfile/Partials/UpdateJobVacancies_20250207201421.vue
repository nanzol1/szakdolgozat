...
const checkMainCategory = (category) => {
    category.subcategories.forEach((subcat) => {
        if(!checkboxes.value.includes('s'+subcat.id)){
            checkboxes.value.push(subcat.id);
        }
    });
}
</script>
        ...
        >
            ...
            <div>
                ...
                <div v-for="cats in categories" :key="cats.id">
                    ...
                    <Checkbox :value="'m'+cats.id" v-model:checked="checkboxes" :id="cats.id">{{ cats.name }}</Checkbox>
                    <div v-for="subcats in cats.subcategories" :key="subcats.id">
                        <label :for="subcats.name">{{ subcats.name }}</label>
                        <Checkbox :value="'s'+subcats.id" v-model:checked="checkboxes" :id="subcats.name" @change="toggleSubcategories(subcats)>{{ subcats.name }}</Checkbox>
                    </div>
                </div>
