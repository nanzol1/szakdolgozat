...
const page = usePage();
const is_company = ref(page.props.auth?.is_company || 0);
const navigation = [
  { name: 'Kezdőlap', href: route('/'), current: page.url === '/' ? true : false },
  { name: 'Állások', href: route('jobs.show'), current: page.url.includes('munkak') ? true : false },
  { name: 'Önéletrajz Készítés', href: route('cv.show'), current: page.url.includes('cvmaker') ? true : false, isVisible: page.props.auth.company ? 'hidden' : ''},
  { name: 'Hirdetésfeladás', href: is_company.value === 1 ? route('cprofile.edit.jobs') : route('register',{type:'munkaltato'}), current: page.url === '/hirdetes/feladas' ? true : false, isVisible:page.props.auth.user ? 'hidden' : '' },
...
<template>
    ...
    >
        <Disclosure as="nav" :class="['text-gray-950 flex flex-wrap w-full justify-center gap-5 !border-solid !border-b !border-black bg-[#C7A07A] p-2 xl:text-xl lg:text-md md:text-xs sm:text-sm',{'rounded-xl border-0': isFixed}]" v-slot="{ open }">
                ...
                <div class="flex w-full items-center justify-center sm:items-stretch sm:justify-start">
                  <div class="hidden lg:ml-3 lg:block">
                    <div class="flex space-x-4">
                      ...
                      :class="[item.current ? 'text-gray-50' : 'hover:text-white transition', item.isVisible , 'xl:px-3 xl:py-2 text-[14px] lg:text-[16px] md:text-[15px] font-medium']" :aria-current="item.current ? 'page' : undefined">{{ item.name }}</Link>
                    </div>
                  </div>
                </div>
