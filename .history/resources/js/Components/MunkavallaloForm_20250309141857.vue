...
<div>
    ...
    <v-form @submit.prevent="validate" enctype="multipart/form-data" ref="formValidator">
        ...
        <div>
            <v-btn class="!bg-[#A67C52] hover:!bg-[#3A2618] hover:scale-105 !text-gray-50" type="submit">Regisztráció</v-btn>
        </div>
        <div class="flex flex-col gap-1 md:gap-2 mt-2 mt-md-4">
            <p>Már van fiókom szeretnék bejelentkezni.</p>
            <div><v-btn class="!bg-[#A67C52] hover:!bg-[#3A2618] hover:scale-105 !text-gray-50" @click="router.visit(route('login'))">Bejelentkezés</v-btn></div>
        </div>
