...
import { NCarousel, NCard, NIcon, sliderProps } from 'naive-ui'
...
onMounted(() => {
    ...
    window.addEventListener('resize', availWidth);
...
watch(availWidth, (newWidth) => {
  availWidth.value = newWidth;
});
                            ...
                            >
                                <Link v-for="job in jobs" :key="job.id" :href="route('jobs.view',{id:job.id})">
                                    <n-card
                                      ...
                                      size="medium"
                                      footer-class="sm:max-h-32 sm:min-h-32 h-auto text-ellipsis overflow-hidden"
